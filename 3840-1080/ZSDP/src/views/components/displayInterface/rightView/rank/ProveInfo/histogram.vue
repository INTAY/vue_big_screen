<template>
  <div class="histogram">
    <div class="btn">
      <btnChange :btnDates="dateBtns" :disabled="isDisabled" @checkDate="Change" />
    </div>
    <!-- 充电桩数（个） -->
    <div class="graphical">
      <Histogram
        ...
        :value="valueData"
        :dataZoom="zoomData"
        style="width:492px;height:230px"
      />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async dietzconstruction() {
      let regId = this.login.regId;
      const { data: res } = await request(
        `/largeScreen/assetInfo/getDevByBuildType/${regId}`
      );
      //地名
